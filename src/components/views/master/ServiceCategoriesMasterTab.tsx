...
import { ServiceCategory, ServiceItem, UserAccount } from '../../../types';
...
interface ServiceCategoriesMasterTabProps {
...
export const ServiceCategoriesMasterTab: React.FC<ServiceCategoriesMasterTabProps> = ({
  categories,
  services,
  currentUser,
  lang,
  ...
  const handleOpenAdd = () => {
    ...
    setFormCode(`SC-${Date.now().toString().slice(-4)}`);
    setFormName('');
    setFormNameMm('');
    setFormDescription('');
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingCategory) {
        await db.serviceCategories.update(editingCategory.id, {
      ...
      setFormError(err?.message || 'Error saving service category');
    }
  };

  const handleToggleActive = async (category: ServiceCategory) => {
    try {
      await db.serviceCategories.update(category.id, {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-gray-200 shadow-xs">
        <div>
          <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
            ...
            <span>{isMm ? 'ဝန်ဆောင်မှု ကဏ္ဍများ စီမံခန့်ခွဲမှု' : 'Service Categories'}</span>
          ...
          <p className="text-xs text-gray-500 mt-1">
            {isMm
              ? 'အနှိပ်၊ စပါ၊ အလှပြင်နှင့် KTV ကဏ္ဍခွဲများ၊ အစဉ်လိုက်စီစဉ်မှုနှင့် မီနူးအုပ်စုများ'
              : 'Categorize treatments and packages for fast navigation and operational reporting.'}
          </p>
        </div>
        ...
        >
          ...
          <span>{isMm ? 'ကဏ္ဍအသစ် သတ်မှတ်ရန်' : 'Add Service Category'}</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-white p-4 rounded-2xl border border-gray-200 shadow-xs">
        <div className="relative">
          ...
          <input
            ...
            placeholder={isMm ? 'ကဏ္ဍကုဒ်၊ အမည်ဖြင့် ရှာရန်...' : 'Search category code, name...'}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-gray-200 bg-slate-50 text-[11px] font-bold text-gray-600 uppercase tracking-wider">
                ...
                <th className="py-3 px-4">{isMm ? 'ဝန်ဆောင်မှုအရေအတွက်' : 'Services Count'}</th>
              ...
              {filteredCategories.length === 0 ? (
                <tr>
                  <td colSpan={7} className="py-8 text-center text-gray-400">
                    {isMm ? 'ဝန်ဆောင်မှုကဏ္ဍ ရှာမတွေ့ပါ' : 'No service categories found.'}
                  </td>
                </tr>
              ) : (
                filteredCategories.map((cat) => {
                  const linkedCount = services.filter(
          ...
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl border border-gray-100 my-8">
            <div className="flex items-center justify-between border-b border-gray-100 pb-3 mb-4">
              <h3 className="text-base font-bold text-gray-900 flex items-center gap-2">
                ...
                <span>
                  {editingCategory
                    ? isMm ? 'ဝန်ဆောင်မှုကဏ္ဍ ပြင်ဆင်ခြင်း' : 'Edit Service Category'
                    : isMm ? 'ဝန်ဆောင်မှုကဏ္ဍ အသစ်ထည့်ခြင်း' : 'Add New Service Category'}
                </span>
              </h3>
              <button
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Massage & Reflexology"
                  className="w-full rounded-xl border border-gray-200 px-3 py-2 text-xs focus:border-amber-500 focus:ring-1 focus:ring-amber-500 outline-none"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="ဥပမာ - အနှိပ်နှင့် အကြောပြင်"
                  className="w-full rounded-xl border border-gray-200 px-3 py-2 text-xs focus:border-amber-500 focus:ring-1 focus:ring-amber-500 outline-none"
                />
              </div>
              ...
              <div className="flex items-center gap-3 pt-1">
                ...
                />
                <label htmlFor="formIsActiveCat" className="text-xs font-medium text-gray-700">
