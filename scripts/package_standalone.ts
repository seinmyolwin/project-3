...
function packagePhase23Standalone() {
  ...
  const tempExe = path.join(rootDir, 'temp_node', 'node.exe');
  ...
  if (fs.existsSync(tempExe)) {
    fs.copyFileSync(tempExe, path.join(releaseDir, 'bin', 'node.exe'));
    console.log('✓ Zero-Install Windows Runtime (bin/node.exe) bundled in release/bin/.');
  } else if (fs.existsSync(path.join(binDir, 'node.exe'))) {
    fs.copyFileSync(path.join(binDir, 'node.exe'), path.join(releaseDir, 'bin', 'node.exe'));
    console.log('✓ Zero-Install Windows Runtime (bin/node.exe) bundled in release/bin/.');
  }
